...
def get_NPC(y_test, y_score):
    tn, fp, fn, tp = confusion_matrix(y_test, y_score).ravel()
    npc =  tn / (tn + fn)
    return npc

def get_PositiveLR(y_test, y_score):
    # sensitivity / (1 - specificity)
    positive_lr = get_Sensitivity(y_test, y_score) / (1 - get_Specificity(y_test, y_score))
    return positive_lr

def get_NegativeLR(y_test, y_score):
    # (1 - sensitivity) / specificity
    negative_lr = (1 - get_Sensitivity(y_test, y_score)) / get_Specificity(y_test, y_score)
    return negative_lr
